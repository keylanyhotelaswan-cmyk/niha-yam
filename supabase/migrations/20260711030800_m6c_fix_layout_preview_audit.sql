-- Fix layout preview print: audit action must be on the allowlist.
-- print.layout_preview_enqueued was rejected by chk_audit_log_m1_actions → generic UI error.
...
BEGIN
  ...
  IF v_layout IS NOT NULL AND (v_layout ? 'paper_width_mm') THEN
    BEGIN
      v_paper := coalesce(nullif((v_layout->>'paper_width_mm')::int, 0), v_paper);
    EXCEPTION WHEN others THEN
      NULL; -- keep previous paper width
    END;
  END IF;
  IF v_paper NOT IN (58, 80) THEN
    v_paper := 80;
  END IF;
  ...
  -- Reuse allowlisted action (chk_audit_log_m1_actions)
