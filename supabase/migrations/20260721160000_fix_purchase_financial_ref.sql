-- Fix purchase (and related) financial refs: next_financial_ref used to fall
-- through to treasury_transfers for unknown types, so PUR-* collisions were
-- not detected and UNIQUE (restaurant_id, reference) on purchases failed.
...
-- Heal purchase counters so next PUR-* is above existing rows.
INSERT INTO public.financial_ref_counters (restaurant_id, ref_type, current_value)
SELECT r.id, 'purchase', public.financial_ref_table_max(r.id, 'purchase', 'PUR')
