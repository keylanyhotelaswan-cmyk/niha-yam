-- Testing: reject qty < 1 in finalize_sale (INVALID_QTY)
-- Was silently coerced via greatest(qty, 1).
...
BEGIN
  ...
  -- Reject non-positive quantities (do not coerce 0 → 1)
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    IF coalesce((v_line->>'quantity')::int, 0) < 1 THEN RAISE EXCEPTION 'INVALID_QTY'; END IF;
  END LOOP;
