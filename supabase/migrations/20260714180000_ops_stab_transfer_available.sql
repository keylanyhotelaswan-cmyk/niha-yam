-- Operational hotfix: align transfer funds check with transferable cash.
-- UI showed m5b_operational_treasury_balance while RPC used treasury_balance only.
-- Live symptom: drawer "available" 40 (operational) but transfer always INSUFFICIENT_FUNDS
-- because approved ledger was negative (-260) while pending cash still +300.
-- Transferable = least(approved ledger, operational) — cannot move unapproved pending cash.
...
BEGIN
  ...
  v_ledger := public.treasury_balance(p_source_treasury_id);
  v_operational := public.m5b_operational_treasury_balance(p_source_treasury_id, v_shift);
  v_available := least(v_ledger, v_operational);
