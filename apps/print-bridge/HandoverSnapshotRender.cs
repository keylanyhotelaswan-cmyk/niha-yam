...
/// <summary>Dedicated ESC/POS render for shift_handover print jobs (OES polish).</summary>
internal static class HandoverSnapshotRender
{
    public static bool TryRender(ClaimedJob job, out byte[] bytes)
    {
        ...
        try
        {
            ...
            doc.DashedRule();
            doc.Line("تفصيل التحصيل", EscPosAlign.Center, fontSize: style.FontBodyPt, bold: true);
            ...
            {
                doc.DashedRule();
                LineKv(doc, "الفرق (Variance)", FormatMoney(v) + " " + cur, style.FontBodyPt, bold: true);
            }
